...
describe('Card Content Component', () => {
  ...
  it('Renders no padding version', () => {
    const {
      ...
      <AptoCardContent noPadding/>
    );

    const node = container.querySelector('.AptoCardContent');
    expect(node!.className).toEqual('AptoCardContent AptoCardContent--noPadding');
  });
  ...
  it('Renders children', () => {
    const {
      ...
      <AptoCardContent><div>foo</div></AptoCardContent>
    );
